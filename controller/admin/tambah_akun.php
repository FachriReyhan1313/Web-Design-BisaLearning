...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        header("Location: ../views/admin/tambah_akun.php?success=1");
    } else {
        header("Location: ../views/admin/tambah_akun.php?error=" . urlencode("Gagal menambahkan akun."));
    }
    $stmt->close();
    $conn->close();
} else {
    header("Location: ../views/admin/tambah_akun.php");
}
?>
